import os
import cv2
from PyQt6.QtWidgets import QApplication, QMainWindow, QWidget, QHBoxLayout, QVBoxLayout, QPushButton, QFileDialog, QProgressBar, QLineEdit, QLabel, QCheckBox, QStyle
...
pipe = StableDiffusionImg2ImgPipeline.from_pretrained(model_id, revision="fp16", torch_dtype=torch.bfloat16, use_auth_token=auth_token)
pipe.to(device)
...
class VideoProcessorThread(QThread):
    ...
    def generate_diffused_frame(self, frame):
        print("Generating diffused frame...")
        with autocast(device):
            ...
            prompt = "weirds universe"
            generator = torch.Generator("cuda").manual_seed(1024)
            images = pipe(prompt=prompt, image=init_image, strength=0.25, guidance_scale=5.5, generator=generator).images
    ...
    def run(self):
        ...
        while cap.isOpened(): # Extract the frames from the video and save them as images
            ret, frame = cap.read()
            if not ret:
                break
            frame_count += 1
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # style prompt input
        self.prompt_input.setStyleSheet("background-color: #FFFFFF; border: 1px solid black; border-radius: 5px; padding: 5px;")
        self.negative_prompt_input.setStyleSheet("background-color: #FFFFFF; border: 1px solid black; border-radius: 5px; padding: 5px;")
        ...
        open_button.setStyleSheet("background-color: #1E90FF; color: white; font-weight: bold; border-radius: 10px; border: 2px solid #1E90FF;" )
        # style hover
    ...
    def handle_media_status_changed(self, status):
        ...
        if status == self.mediaPlayer.MediaStatus.EndOfMedia:
            ...
            if is_video_correct('test2.mp4'):
                ...
                self.mediaPlayer.setSource((QUrl.fromLocalFile("images/ExpandingUniverse2.mp4")))
                ...
                self.handle_media_status_changed(self.mediaPlayer.MediaStatus.EndOfMedia)
    ...
    def open_file_dialog(self):
        ...
        self.video_file_path, _ = QFileDialog.getOpenFileName(self, "Open Video File", "", "Video Files (*.mp4 *.avi)")
